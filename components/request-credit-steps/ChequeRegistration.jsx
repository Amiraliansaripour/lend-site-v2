...
const ChequeRegistration = ({
  ...
  useEffect(() => {
    ...
    mandatoryFiles.chequeImage,
    mandatoryFiles.chequeBackImage,
    mandatoryFiles.promissoryImage,
    mandatoryFiles.salaryDeductionImage,
    optionalFiles,
  ]);
      ...
      <div className='text-right shadow-lg p-6 rounded-2xl bg-white'>
        {guarantees.includes('چک') && (
          <div className='w-full flex flex-col items-start justify-start border border-gray-200 rounded-lg mb-4'>
            ...
            <div
              className={`w-full px-4 pb-4 transition-all duration-300 overflow-hidden ${accordionStates.cheque ? 'max-h-none opacity-100' : 'max-h-0 opacity-0 pb-0'}`}
            >
              {/* Content wrapper */}
              <Alert className='mt-4 w-full mb-6 '>
                ...
                <div className='flex flex-col justify-center md:flex-row md:gap-10 items-start'>
                  ...
                  <span>
                    ...
                    <span className='font-bold text-blue-500 col-span-1'>
                      فناوری اطلاعات راژمان
                    </span>
                  </span>
                </div>
              </Alert>
              ...
              <form onSubmit={handleSubmit(onSubmit, onError)} className='w-full mt-6 grid gap-5'>
                <div className='w-full mb-10 grid gap-8 sm:place-items-center'>
                  <div className='grid gap-4 w-full md:w-2/4'>
                    ...
                    />
                    <span
                      className={`text-sm ${sayadId.length === 16 ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {sayadId.length <= 0 ? null : `${sayadId.length} رقم وارد شده`}
                    </span>
                    {errors.sayadId && (
                      <span className='text-red-500 text-sm mt-1'>{errors.sayadId.message}</span>
                    )}
                  </div>
                  ...
                  <div className='grid gap-4 w-full md:w-2/4'>
                    ...
                    <div
                      ...
                      onDrop={e => handleDrop(e, 'chequeImage')}
                    >
                      {mandatoryFiles.chequeImage?.preview ? (
                        <img
                          src={mandatoryFiles.chequeImage.preview}
                          alt='تصویر چک صیادی'
                          width='60'
                          height='60'
                    ...
                    <Alert>
                      ...
                      <span className='!font-bold text-red-500'>
                        {PriceDisplayWithOutLabel(+guaranteedAmount)}
                      </span>
                      &nbsp; ریال باشد
                    </Alert>
                        ...
                        {uploadProgress.chequeImage.status === 'uploading' && (
                          <div role='status' className='flex items-center justify-center'>
                            ...
                            <span className='text-blue-500 text-sm ml-2'>
                              {uploadProgress.chequeImage.message}
                            </span>
                          </div>
                        )}
                        {uploadProgress.chequeImage.status === 'success' && (
                          <p className='text-green-500 text-sm'>
                            {uploadProgress.chequeImage.message}
                          </p>
                        )}
                        {uploadProgress.chequeImage.status === 'error' && (
                  ...
                  <div className='grid gap-4 w-full md:w-2/4'>
                    ...
                    <div
                      className='border border-gray-300 rounded-lg p-4 w-full text-center cursor-pointer hover:bg-gray-100 transition-colors duration-300'
                      onDragOver={e => e.preventDefault()}
                      onDrop={e => handleDrop(e, 'chequeBackImage')}
                    >
                      {mandatoryFiles.chequeBackImage?.preview ? (
                        <img
                          src={mandatoryFiles.chequeBackImage.preview}
                      ...
                      <input
                        ...
                        accept='image/png, image/jpeg'
                        className='mt-2 w-full text-sm text-gray-600 file:py-2 file:px-4 file:border file:border-gray-300 file:rounded file:bg-gray-50 file:text-gray-700 hover:file:bg-gray-100'
                        onChange={e => handleMandatoryUpload(e, 'chequeBackImage', 107)}
                        value={mandatoryFiles.chequeBackImage ? undefined : ''}
                      />
                    </div>
                      ...
                      <div className='mt-2 text-center'>
                        {uploadProgress.chequeBackImage.status === 'uploading' && (
                          <div role='status' className='flex items-center justify-center'>
                            <SvgSpinner />
                            <span className='text-blue-500 text-sm ml-2'>
                              {uploadProgress.chequeBackImage.message}
                            </span>
                          </div>
                        )}
                        {uploadProgress.chequeBackImage.status === 'success' && (
                          <p className='text-green-500 text-sm'>
                            {uploadProgress.chequeBackImage.message}
                          </p>
                        )}
                        {uploadProgress.chequeBackImage.status === 'error' && (
                          <p className='text-red-500 text-sm'>
                            {uploadProgress.chequeBackImage.message}
                          </p>
                        )}
        ...
        {guarantees.includes('کسر از حقوق') && (
          <div className='w-full flex flex-col items-start justify-start border border-gray-200 rounded-lg mb-4'>
            ...
            <div
              className={`w-full px-4 pb-4 transition-all duration-300 overflow-hidden ${accordionStates.salaryDeduction ? 'max-h-none opacity-100' : 'max-h-0 opacity-0 pb-0'}`}
            >
              {/* Content wrapper */}
              <Alert className='mt-4 w-full mb-6 '>
                <p className='font-bold text-lg text-center mb-6'>
                  لطفاً مدارک مربوط به کسر از حقوق را آپلود کنید:
                </p>
              </Alert>
              ...
              <div className='w-full mt-6 grid gap-5'>
                <div className='w-full mb-10 grid gap-8 sm:place-items-center'>
                  <div className='grid gap-4 w-full md:w-2/4'>
                    <label className='block text-lg font-medium text-gray-700'>
                      تصویر مدرک کسر از حقوق *
                    </label>
                    <p className='text-blue-500'>پسوندهای مجاز: .jpg, .jpeg, .png</p>
                    <div
                      className='border border-gray-300 rounded-lg p-4 w-full text-center cursor-pointer hover:bg-gray-100 transition-colors duration-300'
                      onDragOver={e => e.preventDefault()}
                      onDrop={e => handleDrop(e, 'salaryDeductionImage')}
                    >
                      {mandatoryFiles.salaryDeductionImage?.preview ? (
                        <img
                          src={mandatoryFiles.salaryDeductionImage.preview}
                          alt='تصویر مدرک کسر از حقوق'
                          width='60'
                          height='60'
                          className='mx-auto mb-2 rounded'
                        />
                      ...
                      <input
                        ...
                        accept='image/png, image/jpeg'
                        className='mt-2 w-full text-sm text-gray-600 file:py-2 file:px-4 file:border file:border-gray-300 file:rounded file:bg-gray-50 file:text-gray-700 hover:file:bg-gray-100'
                        onChange={e => handleMandatoryUpload(e, 'salaryDeductionImage', 110)}
                        value={mandatoryFiles.salaryDeductionImage ? undefined : ''}
                      />
                    </div>
                      ...
                      <div className='mt-2 text-center'>
                        {uploadProgress.salaryDeductionImage.status === 'uploading' && (
                          <div role='status' className='flex items-center justify-center'>
                            <SvgSpinner />
                            <span className='text-blue-500 text-sm ml-2'>
                              {uploadProgress.salaryDeductionImage.message}
                            </span>
                          </div>
                        )}
                        {uploadProgress.salaryDeductionImage.status === 'success' && (
                        ...
                        {uploadProgress.salaryDeductionImage.status === 'error' && (
                          <p className='text-red-500 text-sm'>
                            {uploadProgress.salaryDeductionImage.message}
                          </p>
                        )}
        ...
        {guarantees.includes('سفته') && (
          <div className='w-full flex flex-col items-start justify-start border border-gray-200 rounded-lg mb-4'>
            ...
            <div
              className={`w-full px-4 pb-4 transition-all duration-300 overflow-hidden ${accordionStates.promissoryNote ? 'max-h-none opacity-100' : 'max-h-0 opacity-0 pb-0'}`}
            >
              {/* Content wrapper */}
              <Alert className='mt-4 w-full mb-6 '>
                <p className='font-bold text-lg text-center mb-6'>
                  لطفاً مدارک مربوط به سفته را آپلود کنید:
                </p>
                <div className='flex flex-col justify-center md:flex-row md:gap-10 items-start'>
                  <span>مبلغ تضمین: </span>
                  <span className='font-bold text-blue-500 '>
                    {PriceDisplayWithOutLabel(+guaranteedAmount)} ریال
                  </span>
                </div>
              </Alert>
                ...
                <div className='w-full mb-10 grid gap-8 sm:place-items-center'>
                  <div className='grid gap-4 w-full md:w-2/4'>
                    ...
                    <div
                      className='border border-gray-300 rounded-lg p-4 w-full text-center cursor-pointer hover:bg-gray-100 transition-colors duration-300'
                      onDragOver={e => e.preventDefault()}
                      onDrop={e => handleDrop(e, 'promissoryImage')}
                    >
                      {mandatoryFiles.promissoryImage?.preview ? (
                        <img
                          src={mandatoryFiles.promissoryImage.preview}
                          alt='تصویر سفته'
                          width='60'
                          height='60'
                          className='mx-auto mb-2 rounded'
                      ...
                      <input
                        ...
                        accept='image/png, image/jpeg'
                        className='mt-2 w-full text-sm text-gray-600 file:py-2 file:px-4 file:border file:border-gray-300 file:rounded file:bg-gray-50 file:text-gray-700 hover:file:bg-gray-100'
                        onChange={e => handleMandatoryUpload(e, 'promissoryImage', 111)}
                        value={mandatoryFiles.promissoryImage ? undefined : ''}
                      />
                    </div>
                      ...
                      <div className='mt-2 text-center'>
                        {uploadProgress.promissoryImage.status === 'uploading' && (
                          <div role='status' className='flex items-center justify-center'>
                            <SvgSpinner />
                            <span className='text-blue-500 text-sm ml-2'>
                              {uploadProgress.promissoryImage.message}
                            </span>
                          </div>
                        )}
                        {uploadProgress.promissoryImage.status === 'success' && (
                          <p className='text-green-500 text-sm'>
                            {uploadProgress.promissoryImage.message}
                          </p>
                        )}
                        {uploadProgress.promissoryImage.status === 'error' && (
                          <p className='text-red-500 text-sm'>
                            {uploadProgress.promissoryImage.message}
                          </p>
                        )}
