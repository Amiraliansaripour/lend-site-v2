...
const AcceptByUser = ({ onBack, user, ruleText }) => {
  ...
  useEffect(() => {
    const fetchRequestPreview = async () => {
      ...
      try {
        ...
        setRequestData(data);
        const tempRecivedData =
          data.creditAmount -
          data.creditAmount / Math.floor(data.planFirstSystemFee + data.planFirstBankFee);
  ...
  if (error) {
    return (
      <div className='w-full bg-white custom-shadow rounded-2xl gap-4 p-4 text-center text-red-600 font-bold'>
        {error}
      </div>
    );
  }
      ...
      {loading ? (
        ...
        <>
          ...
          <div className='text-center'>
            <h2 className='font-extrabold text-3xl text-gray-900 mb-3 border-b-2 border-gold-primary-700 pb-2 inline-block'>
              جزئیات درخواست شما
            </h2>
                ...
                <div className=''>
                  ...
                  <div className='flex sm:flex-row flex-col  justify-between mt-2'>
                    {userImage?.map((res, index) => (
                      <img
                        key={index}
                        src={`data:image/jpeg;base64,${res?.file}`}
                        alt={`User Document ${index + 1}`}
                        className='w-24 lg:w-28 h-fit my-1 cursor-pointer hover:opacity-80 transition-opacity rounded-md'
                        onClick={() => openImageModal(`data:image/jpeg;base64,${res?.file}`)}
            ...
            <div className='bg-gradient-to-br from-green-50 to-teal-50 p-6 rounded-xl shadow-lg border border-green-100 transform hover:scale-105 transition-transform duration-300'>
              ...
              <div className='space-y-2'>
                ...
                <p className='flex justify-between items-center text-base'>
                  ...
                  {chequeFileImageBase64 ? (
                    <img
                      src={createBase64ImageUrl(chequeFileImageBase64)}
                      alt='تصویر چک صیادی'
                      className='w-24 h-24 object-cover rounded-md cursor-pointer hover:opacity-80 transition-opacity'
                      onClick={() => openImageModal(createBase64ImageUrl(chequeFileImageBase64))}
                      title='برای مشاهده تصویر کامل کلیک کنید'
                    />
                  ) : (
                ...
                <p className='flex justify-between items-center text-base'>
                  ...
                  {backChequeFileImageBase64 ? (
                    <img
                      src={createBase64ImageUrl(backChequeFileImageBase64)}
                      alt='تصویر چک صیادی'
                      className='w-24 h-24 object-cover rounded-md cursor-pointer hover:opacity-80 transition-opacity'
                      onClick={() =>
                        openImageModal(createBase64ImageUrl(backChequeFileImageBase64))
                      }
                      title='برای مشاهده تصویر کامل کلیک کنید'
                    />
                  ) : (
                ...
                <p className='flex justify-between items-center text-base'>
                  ...
                  {promissoryImage ? (
                    <img
                      src={createBase64ImageUrl(promissoryImage)}
                      alt='تصویر  سفته'
                      className='w-24 h-24 object-cover rounded-md cursor-pointer hover:opacity-80 transition-opacity'
                      onClick={() => openImageModal(createBase64ImageUrl(promissoryImage))}
                      title='برای مشاهده تصویر کامل کلیک کنید'
                    />
                  ) : (
                    <span className='text-gray-700'>تصویری وجود ندارد</span>
                ...
                <p className='flex justify-between items-center text-base'>
                  ...
                  {deductionSalaryFileImage ? (
                    <img
                      src={createBase64ImageUrl(deductionSalaryFileImage)}
                      alt='تصویر گواهی کسر از حقوق'
                      className='w-24 h-24 object-cover rounded-md cursor-pointer hover:opacity-80 transition-opacity'
                      onClick={() => openImageModal(createBase64ImageUrl(deductionSalaryFileImage))}
                      title='برای مشاهده تصویر کامل کلیک کنید'
                    />
                  ) : (
                    <span className='text-gray-700'>تصویری وجود ندارد</span>
              ...
              <div className='bg-gradient-to-br from-yellow-50 to-orange-50 p-6 rounded-xl shadow-lg border border-yellow-100 col-span-1 lg:col-span-2 transform hover:scale-105 transition-transform duration-300'>
                <h3 className='font-bold text-xl text-orange-700 mb-4 flex items-center'>
                  ...
                  >
                    <path
                      strokeLinecap='round'
                      strokeLinejoin='round'
                      d='M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.592 1L21 12m-2.208 9L11 19m-9.792-7L1 5m14.828 4.828l-8.485 8.485M12 10V5m0 14v-5'
                    />
                  </svg>
                  اطلاعات درآمدی
                </h3>
                <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                  <p className='flex flex-col text-base'>
                    ...
                    <span className='whitespace-nowrap text-gray-700 font-semibold'>
                      {requestData?.incomeInfo.income
                  ...
                  <p className='flex flex-col text-base'>
                    ...
                    <span className='whitespace-nowrap text-gray-700 font-semibold'>
                      {requestData?.incomeInfo.payAbility
                        ? `${requestData?.incomeInfo.payAbility.toLocaleString()} ریال`
                        : 'نامشخص'}
                    </span>
                  </p>
                  <p className='flex flex-col text-base'>
            ...
            {requestData?.plan && (
              <div className='bg-gradient-to-br from-purple-50 to-pink-50 p-6 rounded-xl shadow-lg border border-purple-100 transform hover:scale-105 transition-transform duration-300'>
                <h3 className='font-bold text-xl text-purple-700 mb-4 flex items-center'>
                  <svg
                    xmlns='http://www.w3.org/2000/svg'
                    className='h-6 w-6 ml-2 text-purple-500'
                    fill='none'
                    viewBox='0 0 24 24'
                    stroke='currentColor'
                    strokeWidth='2'
                ...
                <div className='space-y-2'>
                  ...
                  <p className='flex justify-between items-center text-base'>
                    ...
                    <span className='text-gray-700'>
                      {requestData?.plan.planPeriod || 'نامشخص'}
                    </span>
                  </p>
                  <p className='flex justify-between items-center text-base'>
                    <span className='font-medium text-gray-800'>ضمانت‌های طرح:</span>
                    <span className='text-gray-700'>
                      {requestData?.plan.planGuarantees || 'نامشخص'}
                    </span>
                  </p>
                </div>
              </div>
            ...
            {requestData?.planIsInvoiceRequired && requestData?.invoiceFileImage && (
              <div className='bg-gradient-to-br from-amber-50 to-yellow-50 p-6 rounded-xl shadow-lg border border-amber-100 transform hover:scale-105 transition-transform duration-300'>
                <h3 className='font-bold text-xl text-amber-700 mb-4 flex items-center'>
                  <svg
                    xmlns='http://www.w3.org/2000/svg'
                    className='h-6 w-6 ml-2 text-amber-500'
                    fill='none'
                    viewBox='0 0 24 24'
                    stroke='currentColor'
                    strokeWidth='2'
                ...
                <div className='space-y-2'>
                  <p className='flex justify-between items-center text-base'>
                    ...
                    {invoiceFileImageBase64 ? (
                      <img
                        src={createBase64ImageUrl(invoiceFileImageBase64)}
                        alt='تصویر پیش فاکتور'
                        className='w-24 h-24 object-cover rounded-md cursor-pointer hover:opacity-80 transition-opacity'
                        onClick={() => openImageModal(createBase64ImageUrl(invoiceFileImageBase64))}
                        title='برای مشاهده تصویر کامل کلیک کنید'
                      />
                    ) : requestData?.invoiceFileImage ? (
                      <a
                        href={requestData?.invoiceFileImage}
                        target='_blank'
                        rel='noopener noreferrer'
            ...
            <div className='bg-gradient-to-br from-red-50 to-orange-50 p-6 rounded-xl shadow-lg border border-red-100 transform hover:scale-105 transition-transform duration-300'>
              ...
              <div className='space-y-2'>
                ...
                <p className='flex justify-between items-center text-base'>
                  ...
                  <span className='whitespace-nowrap text-gray-700 font-semibold'>
                    {requestData?.period
                      ? `${requestData.period.toLocaleString('fa')} ماه`
                      : 'نامشخص'}
                  </span>
                </p>
                <p className='flex justify-between items-center text-base'>
                  ...
                  <span className='whitespace-nowrap text-gray-700 font-semibold'>
                    {PriceDisplayWithOutLabel(requestData?.loanDetailAmount)}ریال
                  </span>
                </p>
                <p className='flex justify-between items-center text-base'>
                  <span className='font-medium text-gray-800'>مبلغ اعتبار:</span>
                  <span className='whitespace-nowrap text-gray-700 font-semibold'>
                    {PriceDisplayWithOutLabel(requestData?.creditAmount)}ریال
                  </span>
                </p>
                ...
                <p className='flex justify-between items-center text-base'>
                  ...
                  <span className='whitespace-nowrap text-gray-700 font-semibold'>
                    {PriceDisplayWithOutLabel(requestData?.feeAmount)}ریال
                  </span>
                </p>
                <p className='flex justify-between items-center text-base'>
                  <span className='font-medium text-gray-800'>اعتبار دریافتی :</span>
                  <span className='whitespace-nowrap text-gray-700 font-semibold'>
                    {PriceDisplayWithOutLabel(recieveAmount)}ریال
                  </span>
                </p>
          ...
          <div className='flex justify-center gap-6 mt-10 md:mt-12 flex-col items-center'>
            ...
            <div className='flex items-center mb-6'>
              <input
                ...
                className='h-5 w-5 text-gold-primary-900 border-gray-300 rounded focus:ring-gold-primary-700 focus:ring-2'
              />
              <label
                htmlFor='terms-checkbox'
                className='mx-3 text-xs md:text-lg font-medium text-gray-700 cursor-pointer'
              >
              ...
              {ruleText && (
                <button
                  ...
                  className='mr-4 text-blue-600 hover:text-blue-800 underline text-sm font-medium'
                >
          ...
          {isImageModalOpen && (
            ...
            >
              <div className='relative max-w-4xl max-h-screen p-4'>
                <button
                  onClick={closeImageModal}
                  className='absolute top-2 right-2 text-white text-3xl hover:text-gray-300 z-10 bg-black bg-opacity-50 rounded-full w-10 h-10 flex items-center justify-center'
                >
                  ×
                </button>
                <img
