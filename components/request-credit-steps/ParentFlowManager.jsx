...
const ParentFlowManager = ({ userId, requestId }) => {
  ...
  const handleOtpVerifySuccess = responseDataFromInquiry => {
    console.log('Parent: OTP Verified Successfully. Proceeding to load data...');
    setShowOtpModal(false);
  ...
  useEffect(() => {
    if (userId && requestId) {
      ...
      console.warn('Parent: userId or requestId is missing. Cannot start the flow.');
  ...
  if (currentStep === 'checking_otp_status') {
    return <div className='text-center p-4'>در حال بررسی وضعیت کد تایید...</div>;
  }
  ...
  if (currentStep === 'loading_data') {
    return <div className='text-center p-4'>در حال بارگذاری اطلاعات شما...</div>;
  }
