...
const UserInformation = ({ onNext, onCancellation, user, requestId, isEditMode }) => {
  ...
  useEffect(() => {
    if (attachemnts && attachemnts.length > 0 && !showUploadSection) {
      ...
      const allFilesUploadedSuccessfully = mandatoryFiles.birthCertificate && mandatoryFiles.nationalCardFront && uploadProgress.birthCertificate?.status === "success" && uploadProgress.nationalCardFront?.status === "success";
  ...
  const onSubmit = async (data) => {
    ...
    const shouldUploadNewFiles = !attachemnts || attachemnts.length === 0 || (showUploadSection && Object.keys(uploadedAttachmentIds).length > 0);
    ...
    if (shouldUploadNewFiles) {
      ...
      for (const key of requiredFileKeys) {
        setIsLoading(true);
        if (key !== "nationalCardBack" && (!mandatoryFiles[key] || uploadProgress[key]?.status !== "success" || !uploadedAttachmentIds[key])) {
          let fileName;
          switch (key) {
      ...
      try {
        ...
        if (showUploadSection && attachemnts && attachemnts.length > 0) {
          try {
            const deleteResponse = await axiosInstance.delete(`/api/v1/User/DeleteUserAttachments/${userId}`);
        ...
        const updateResponse = await axiosInstance.put(`/api/v1/User/Upload/${userId}`, { attachmentIdsToSend });

        if (updateResponse.status === 200 && updateResponse.data.isSuccess) {
          setIsLoading(false);
          const validationResponse = await axiosInstance.get(`/api/v1/Request/UserIdentityInfo/${requestId}`, {
            headers: {
              accept: "text/plain",
            },
          });
          ...
          if (validationResponse.data.isSuccess === true) {
            ...
            toast.success("اطلاعات هویتی با موفقیت تایید و مرحله بعد فعال شد.", { className: "rtl-toast-message" });
      ...
      try {
        ...
        if (validationResponse.data.isSuccess === true) {
          ...
          toast.error(validationResponse.data.message || "اطلاعات هویتی تایید نشد.", { className: "rtl-toast-message" });
        }
      } catch (error) {
  ...
  const FileUploader = ({ label, fileKey }) => (
    <div className="w-full">
      <label className="text-lg font-medium text-gray-700 flex items-center gap-2 mb-4">تصویر {label} خود را بارگذاری کنید.</label>
      <div className="border mt-2 border-gray-300 rounded-lg p-4 w-full text-center cursor-pointer hover:bg-gray-100 transition-colors duration-300 relative" onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDrop(e, fileKey)}>
        {/* نمایش پیش‌نمایش تصویر در صورت وجود */}
        {mandatoryFiles[fileKey]?.preview && mandatoryFiles[fileKey]?.format.startsWith("image/") ? (
          <>
            <img src={mandatoryFiles[fileKey].preview} alt={label} width="60" height="60" className="mx-auto mb-2 rounded object-contain" />
            <p className="text-sm text-gray-600">{mandatoryFiles[fileKey].file?.name}</p>
            {/* دکمه حذف فایل */}
            <button type="button" onClick={() => clearFile(fileKey)} className="absolute top-2 left-2 bg-red-500 text-white rounded-full p-1 text-xs hover:bg-red-600" aria-label={`حذف ${label}`}>
              &times;
            </button>
          </>
        ) : (
        ...
        <input type="file" id={`file-input-${fileKey}`} ref={fileInputRefs[fileKey]} className="mt-2 w-full text-sm text-gray-600 file:py-2 file:px-4 file:border file:border-gray-300 file:rounded file:bg-gray-50 file:text-gray-700 hover:file:bg-gray-100" onChange={(e) => handleFileUpload(e, fileKey)} accept="image/png, image/jpeg" />
          ...
          <div className="mt-2 text-center">
            {uploadProgress[fileKey].status === "uploading" && (
              <div role="status" className="flex items-center justify-center">
                <SvgSpinner />
                <span className="text-blue-500 text-sm ml-2">{uploadProgress[fileKey].message}</span>
              </div>
            )}
            {uploadProgress[fileKey].status === "success" && <p className="text-green-500 text-sm">{uploadProgress[fileKey].message}</p>}
            {uploadProgress[fileKey].status === "error" && <p className="text-red-500 text-sm">{uploadProgress[fileKey].message}</p>}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div>
              ...
              <div dir="rtl" className="col-span-2">
                {fields.slice(0, 5).map((field, index) => (
                  <div key={index} className="flex flex-col w-full mt-6">
                    <label htmlFor={field.name} className="text-sm font-bold text-black relative top-2 mb-0 mr-2 px-1 bg-white w-max">
                      {field.label}: *
                    </label>
                    <input
                      {...register(field.name, {
        ...
        <div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col justify-center items-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 justify-items-center items-center w-full max-w-4xl">
                <div className="relative flex justify-center ">
                  {returnPhotoBase64(102) ? (
                    <div className="relative">
                      <img src={`data:image/jpeg;base64,${returnPhotoBase64(102)}`} alt="User Document " className="w-40 h-40 object-cover rounded-lg cursor-pointer hover:opacity-80 transition-opacity shadow-md" onClick={() => openImageModal(`data:image/jpeg;base64,${returnPhotoBase64(102)}`)} />
                      <button type="button" onClick={() => removePhotoSingle(attachemnts?.find((item) => item.attachmentType === 102)?.id)} className="absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600 transition-colors" aria-label="حذف شناسنامه">
                        <MdClose />
                      </button>
                    </div>
                  ) : (
                ...
                <div className="relative flex justify-center ">
                  {returnPhotoBase64(100) ? (
                    <div className="relative">
                      <img src={`data:image/jpeg;base64,${returnPhotoBase64(100)}`} alt="User Document" className="w-40 h-40 object-cover rounded-lg cursor-pointer hover:opacity-80 transition-opacity shadow-md" onClick={() => openImageModal(`data:image/jpeg;base64,${returnPhotoBase64(100)}`)} />
                      <button type="button" onClick={() => removePhotoSingle(attachemnts?.find((item) => item.attachmentType === 100)?.id)} className="absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600 transition-colors" aria-label="حذف روی کارت ملی">
                        <MdClose />
                      </button>
                    </div>
                  ) : (
                ...
                <div className="relative flex justify-center ">
                  {returnPhotoBase64(101) ? (
                    <div className="relative">
                      <img src={`data:image/jpeg;base64,${returnPhotoBase64(101)}`} alt="User Document " className="w-40 h-40 object-cover rounded-lg cursor-pointer hover:opacity-80 transition-opacity shadow-md" onClick={() => openImageModal(`data:image/jpeg;base64,${returnPhotoBase64(101)}`)} />
                      <button type="button" onClick={() => removePhotoSingle(attachemnts?.find((item) => item.attachmentType === 101)?.id)} className="absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600 transition-colors" aria-label="حذف گواهی کسر از حقوق">
                        <MdClose />
                      </button>
                    </div>
                  ) : (
              ...
              <div className="mt-6">
                <button type="button" onClick={handleUploadNewDocuments} className="px-6 py-2 bg-purple-primary text-white rounded-lg  transition-colors">
                  بارگذاری مدارک جدید
                </button>
              </div>
              {showUploadSection && (
                <div className="mt-8 border-t pt-6">
                  ...
                  <p className="text-gray-600 mb-6">اگر می‌خواهید مدارک جدیدی بارگذاری کنید، فایل‌های زیر را انتخاب کنید.</p>
            ...
            <div>
              ...
              <p className="text-gray-600 mb-6">لطفاً تصاویر شناسنامه، روی کارت ملی و گواهی کسر از حقوق را بارگذاری کنید. تمامی فایل‌ها باید با فرمت تصویری (مانند JPG, PNG) و حجم کمتر از 3 مگابایت باشند.</p>
              <div dir="rtl" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <FileUploader label="شناسنامه" fileKey="birthCertificate" />
                <FileUploader label="روی کارت ملی" fileKey="nationalCardFront" />
                <FileUploader label="گواهی کسر از حقوق" fileKey="nationalCardBack" />
      ...
      {showImageModal && selectedImage && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50" onClick={closeImageModal}>
          <div className="relative max-w-4xl max-h-full p-4">
            <img src={selectedImage} alt="تصویر بزرگ شده" className="max-w-full max-h-full object-contain" onClick={(e) => e.stopPropagation()} />
            <button onClick={closeImageModal} className="absolute top-4 right-4 text-white bg-black bg-opacity-50 rounded-full w-8 h-8 flex items-center justify-center hover:bg-opacity-75 transition-all">
              ×
            </button>
          </div>
        </div>
      ...
      {showConfirmationModal && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-gray-600 mb-6 text-center leading-relaxed">با بارگذاری مدارک جدید، تمامی مدارک قبلی شما حذف خواهد شد. آیا مطمئن هستید که می‌خواهید ادامه دهید؟</p>
            <div className="flex gap-4 justify-center">
              <button onClick={confirmUploadNewDocuments} className="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
                بله، ادامه دهید
              </button>
              <button onClick={cancelUploadNewDocuments} className="px-6 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition-colors">
                انصراف
              </button>
            </div>
          </div>
      ...
      {isEditMode ? (
        ...
        <NavigationButton onNext={handleSubmit(onSubmit)} onCancel={handleActualCancellation} isNextDisabled={isButtonDisabled || isLoading} isFirstStep={false} />
      )}
    </div>
  );
};
