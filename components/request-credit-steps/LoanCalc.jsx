...
const LoanCalc = ({ onNext, setRequestId, user, requestId, isEditMode }) => {
    ...
    try {
      ...
      if (fetchedPlans.length > 0) {
        ...
        try {
          ...
          axiosInstance
            .get(`/api/v1/Plan/Get/${firstPlan?.id}`)
            .then(res => setPlanGuarantees(res?.data?.data?.guarantees));
        }
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (selectedPlanDetails && value) {
      const calculate = async () => {
        const result = await calculatePMT(
          value,
          selectedPlanDetails.duringBankFee + selectedPlanDetails.duringSystemFee,
          selectedPlanDetails.period,
        );
        let firstPlansToghater =
          selectedPlanDetails.firstBankFee + selectedPlanDetails.firstSystemFee;
  ...
  const installmentAmount = loanCalculation
    ...
    : Math.floor(calculateInstallment(value, duration));
    ...
    <>
      <div>
              ...
              <div className='flex flex-col'>
                <div className='relative mb-2'>
                  ...
                  >
                    <div
                      className={cn(
                        'text-[#3F455D]  sm:pl-3 py-1 rounded-lg text-xs sm:text-sm font-bold relative',
                      )}
                    >
                      {PriceDisplayWithOutLabel(value)}
                    </div>
                  </div>
                </div>
                <input
                  ...
                  min={
                    selectedPlan
                      ? convertRialToToman(selectedPlan?.minAmount.toString())
                      : '20000000'
                  }
                  max={
                    selectedPlan
                      ? convertRialToToman(selectedPlan?.maxAmount.toString())
                      : '200000000'
                  }
                  step='10000000'
                  value={value}
                ...
                <div className='text-sm sm:text-base lg:text-lg text-purple-darker flex justify-between w-full pt-2'>
                  <div>
                    {' '}
                    {selectedPlan
                      ? PriceDisplayWithOutLabel(selectedPlan?.maxAmount)
                      : PriceDisplayWithOutLabel(200000000)}
                  </div>
                  <div>
                    {selectedPlan
                      ? PriceDisplayWithOutLabel(selectedPlan?.minAmount)
                      : PriceDisplayWithOutLabel(200000000)}
                  </div>
                </div>
              </div>
                    ...
                    <>
                      {item.isActive && (
                        <Button
                          ...
                          active={item?.id === selectedPlan?.id}
                          key={item?.name}
                        >
                          {item?.name}
                        </Button>
        ...
        <CreditModal
          ...
          price={selectedPlan?.documentAmount}
          setIsOpen={setCreditModalOpen}
          isCheckRequired={selectedPlan?.guarantees?.includes('چک')}
          title='نکات قابل توجه جهت درخواست اعتبار'
          onConfirm={onSubmit}
        />
        {isEditMode ? (
          <NavigationButtons
          ...
          <NavigationButtons
            onNext={() => setCreditModalOpen(true)}
            nextLabel='مرحله بعد'
            cancelLabel='بازگشت'
            isFirstStep
          />
