...
const ProformaInvoice = ({ onNext, onBack, isEditMode }) => {
  ...
  const onSubmit = async (data) => {
    if (!mandatoryFiles.proformaInvoice || uploadProgress.proformaInvoice?.status !== "success") {
      toast.error("لطفاً تصویر پیش‌فاکتور را آپلود کرده و از موفقیت آن اطمینان حاصل کنید.", { className: "rtl-toast-message" });
      return;
    }
    ...
    if (!attachmentId) {
      toast.error("شناسه فایل پیوست یافت نشد. لطفاً منتظر آپلود کامل فایل باشید.", { className: "rtl-toast-message" });
      return;
    }
          ...
          <div dir="rtl" className="grid grid-cols-1 gap-10">
            <div className="md:flex gap-6">
              <div className="w-full">
                ...
                <div className="border mt-2 border-gray-300 rounded-lg p-4 w-full text-center cursor-pointer hover:bg-gray-100 transition-colors duration-300 relative" onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDrop(e, "proformaInvoice")}>
                  {mandatoryFiles.proformaInvoice?.preview ? (
                    <>
                      <img src={mandatoryFiles.proformaInvoice.preview} alt="پیش فاکتور" width="60" height="60" className="mx-auto mb-2 rounded object-contain cursor-pointer hover:opacity-80 transition-opacity" onClick={() => openImageModal(mandatoryFiles.proformaInvoice.preview)} />
                      <p className="text-sm text-gray-600">{mandatoryFiles.proformaInvoice.file?.name}</p>
                    </>
                  ) : (
                    <span className="block text-sm text-gray-500 mb-2">فایل را بکشید و رها کنید یا کلیک کنید</span>
                  )}

                  <input type="file" accept="image/png, image/jpeg" id="file-input-proformaInvoice" className="mt-2 w-full text-sm text-gray-600 file:py-2 file:px-4 file:border file:border-gray-300 file:rounded file:bg-gray-50 file:text-gray-700 hover:file:bg-gray-100" onChange={(e) => handleMandatoryUpload(e, "proformaInvoice")} />
                    ...
                    <div className="mt-2 text-center">
                      {uploadProgress.proformaInvoice.status === "uploading" && (
                        <div role="status" className="flex items-center justify-center">
                          <SvgSpinner />
                          <span className="text-blue-500 text-sm ml-2">{uploadProgress.proformaInvoice.message}</span>
                        </div>
                      )}
                      {uploadProgress.proformaInvoice.status === "success" && <p className="text-green-500 text-sm">{uploadProgress.proformaInvoice.message}</p>}
                      {uploadProgress.proformaInvoice.status === "error" && <p className="text-red-500 text-sm">{uploadProgress.proformaInvoice.message}</p>}
                    </div>
                  )}
                </div>

                <p className="mt-6 text-gray-500">فاکتور بارگذاری شده میبایست سربرگ دار و دارای مهر رسمی فروشگاه باشد. همچنین درج نام و نام خانوادگی خریدار، کد ملی خریدار، تاریخ صدور فاکتور، شماره فاکتور و مبلغ دقیق خرید به صورت کاملا واضح و خوانا ضروری میباشد. </p>
              </div>
            </div>
          </div>
        </div>
      ...
      {isImageModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50" onClick={closeImageModal}>
          <div className="relative max-w-4xl max-h-screen p-5 bg-white rounded-xl">
            <button onClick={closeImageModal} className="absolute  top-1 right-1 text-black text-3xl font-bold hover:text-gray-800 z-10">
              ×
            </button>
            <img src={selectedImage} alt="پیش فاکتور بزرگ شده" className="max-w-full max-h-full object-contain rounded-lg" onClick={(e) => e.stopPropagation()} />
          </div>
        </div>
      )}
    </div>
