...
const IncomeInformation = ({ onNext, onBack, user, userIncom, setUserIncom, isEditMode }) => {
  ...
  const onSubmit = async data => {
    ...
    const numericInstallmentPayment = installmentPayment
      ? Number(installmentPayment.replace(/,/g, ''))
      : 0;
            ...
            <div className='grid col-span-2 grid-cols-2 row-span-1'>
              <div className='md:flex gap-8 col-span-2 mb-6'>
                ...
                <div className='flex flex-col col-span-1 w-full mt-6'>
                    ...
                    rules={{
                      validate: value => {
                        ...
                        if (
                          value &&
                          payAbilityValue &&
                          Number(value.replace(/,/g, '')) <= Number(payAbilityValue)
                        ) {
                  ...
                  />
                  {errors.income && (
                    <span className='text-red-500 text-xs'>{errors.income.message}</span>
                  )}
                </div>

                {/* میزان اقساط ماهیانه */}
                <div className='flex flex-col w-full mt-6'>
                  <label className='text-sm font-bold text-black relative top-2 mb-0 mr-2 px-1 bg-white w-max'>
                    میزان اقساط ماهیانه شما (تومان) :
                  </label>
                  <Controller
                    name='payAbility'
                    control={control}
                  ...
                  />
                  {errors.payAbility && (
                    <span className='text-red-500 text-xs'>{errors.payAbility.message}</span>
                  )}
                </div>
              </div>
              ...
              <div className='col-span-2 md:flex gap-6'>
                ...
                <div className='w-full'>
                  ...
                  <div
                    className='border border-gray-300 rounded-lg p-4 w-full text-center cursor-pointer hover:bg-gray-100 transition-colors duration-300 relative'
                    onDragOver={e => e.preventDefault()}
                    onDrop={e => handleDrop(e, 'AccountTurnover')}
                  >
                    {/* نمایش پیش‌نمایش فایل */}
                    {mandatoryFiles.AccountTurnover && mandatoryFiles.AccountTurnover.preview ? (
                      <>
                        <img
                          src={mandatoryFiles.AccountTurnover.preview}
                          alt='گردش حساب'
                          width='60'
                          height='60'
                        ...
                        <p className='text-sm text-gray-600'>
                          {mandatoryFiles.AccountTurnover.file?.name}
                        </p>
                      </>
                    ) : (
  ...
  '
                  ...
                  {uploadProgress.AccountTurnover && (
                    <div className='mt-2 text-center'>
                      {uploadProgress.AccountTurnover.status === 'uploading' && (
                        <div role='status' className='flex items-center justify-center'>
                          ...
                          <span className='text-blue-500 text-sm ml-2'>
                            {uploadProgress.AccountTurnover.message}
                          </span>
                        </div>
                      )}
                      ...
                      {uploadProgress.AccountTurnover.status === 'error' && (
                ...
                <div className='w-full'>
                  ...
                  <div
                    className='border border-gray-300 rounded-lg p-4 w-full text-center cursor-pointer hover:bg-gray-100 transition-colors duration-300 relative'
                    onDragOver={e => e.preventDefault()}
                    onDrop={e => handleDrop(e, 'SalarySlip')}
                  >
                    {/* نمایش پیش‌نمایش فایل */}
                    {mandatoryFiles.SalarySlip && mandatoryFiles.SalarySlip.preview ? (
                      <>
                        <img
                          src={mandatoryFiles.SalarySlip.preview}
                          alt='فیش حقوقی'
                          width='60'
                          height='60'
                          className='mx-auto mb-2 rounded'
                        />
                        <p className='text-sm text-gray-600'>
                          {mandatoryFiles.SalarySlip.file?.name}
                        </p>
                      </>
                    ) : (
                      <span className='block text-sm text-gray-500 mb-2'>
                    ...
                    <div className='mt-2 text-center'>
                      {uploadProgress.SalarySlip.status === 'uploading' && (
                        <div role='status' className='flex items-center justify-center'>
                          <SvgSpinner />
                          <span className='text-blue-500 text-sm ml-2'>
                            {uploadProgress.SalarySlip.message}
                          </span>
                        </div>
                      )}
                      ...
                      {uploadProgress.SalarySlip.status === 'error' && (
                        <p className='text-red-500 text-sm'>{uploadProgress.SalarySlip.message}</p>
                      )}
                    </div>
                  )}
                </div>
        ...
        {!isFormValid && (
          <div className='mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg'>
            ...
            <ul className='text-sm text-yellow-700 space-y-1'>
              ...
              {(!mandatoryFiles.AccountTurnover ||
                uploadProgress.AccountTurnover?.status !== 'success') && (
                <li className='flex items-center'>
                  <span className='w-2 h-2 bg-yellow-400 rounded-full ml-2'></span>
