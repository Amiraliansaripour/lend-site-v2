...
const FinotechValidation = ({ onNext, user, requestId, isEditMode }) => {
  ...
  const checkValidationData = () => {
    try {
      if (requestData?.lifeStatus === true && requestData?.chequeColorStatus === 1 && requestData?.isBlocked !== true && requestData?.over18 && requestData?.facilityDeferred === false && requestData?.guarantyDeferred === false) {
        console.log("here");
        return true;
      } else {
        return false;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full gap-8">
              {isLoadingProcess || !requestData ? (
                ...
                <>
                  ...
                  {renderCard(requestData?.chequeColorStatus === 1, "فاقد چک برگشتی", "دارای چک برگشتی", "چک برگشتی")}
                  {renderCard(!requestData?.isBlocked, "نیست", "است", "لیست سیاه بانکی")}
                  {renderCard(requestData?.over18, "است", "نیست", "بالای 18 سال")}
                  {renderCard(requestData?.facilityDeferred === false, "ندارد", "دارد", "تسهیلات معوق")}
                  {renderCard(requestData?.guarantyDeferred === false, "ندارد", "دارد", "ضمانت های معوق")}
                </>
              )}
            </div>
          </div>
