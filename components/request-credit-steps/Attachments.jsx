...
const Attachments = ({ onBack, onNext, user, requestId }) => {
  ...
  const uploadFilesToAPI = async () => {
    ...
    optionalFiles.forEach((file, index) => {
      if (file) {
        const blob = fetch(`data:${file.format};base64,${file.file}`).then(res => res.blob());
  ...
  const handleSubmit = async () => {
    ...
    if (!mandatoryFiles.idCardBack || !mandatoryFiles.idCardFront || !mandatoryFiles.signature) {
  ...
  return (
    <div className='p-4'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        ...
        <div>
          <label className='block text-lg font-medium mb-2 text-gray-700'>گواهی کسر از حقوق</label>
          ...
          {uploadProgress.idCardBack && (
            <div className='mt-2 text-center'>
              ...
              {uploadProgress.idCardBack === 'error' && (
                <p className='text-red-500 text-sm'>مشکلی پیش آمد، مجدداً تلاش کنید.</p>
              )}
            </div>
          )}
        </div>
        ...
        <div>
          <label className='block text-lg font-medium mb-2 text-gray-700'>رویه کارت ملی</label>
          <p className='text-blue-500'>پسوندهای مجاز: .jpg, .jpeg, .png</p>
        ...
        {/* Mandatory File: امضا */}
        <div>
          <label className='block text-lg font-medium mb-2 text-gray-700'>امضا</label>
          <p className='text-blue-500'>پسوندهای مجاز: .jpg, .jpeg, .png</p>
